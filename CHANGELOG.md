...
- generate MD files 
	- started: done basic template and ontology one
	- @todo class, prop etc...
	- can do a tree?  yes use ``` and outsup simple termnial tree structure
...
- added pygments to dependencies
	- sudo pip install pygments -U [v 2.2 for turtle lexer]
...
August 11, 2016
---------------------------------
- added viz_splitter_multi to viz selection routine
	- split logic so to handle multi file saving
- added inferred properties to viz_splitter_multi
	- layout still not very good @todo improve
...
- moved all splitter templates to /splitter/..
	- updated setup.py and django set up too
- added all entities to splitter viz
	- removed all splitter to selection tool



August 7, 2016
---------------------------------
- started testing adding namespaces to ontology entity, so to make ontology annotations look better
	- done and it seem to work ok
...
July 28, 2016
---------------------------------
- improved new splitter view
	- http://methvin.com/splitter/ [old release]
	- https://github.com/e1ven/jQuery-Splitter [updated for latest jquery]


July 27, 2016
---------------------------------
- added -o option for exporting to a file
	- eg python -m ontospy.ontospy -v -o ~/Desktop/



July 26, 2016
---------------------------------
- added simple graph family tree
...
July 19, 2016
---------------------------------
- fixed bug with dendogram json links
	- various other layout improvements to dendogram
...
June 19, 2016
---------------------------------
- added standard way to add/test viz
	- eg >python -m ontospy.viz.viz_html
- simplified way VISUALIZATIONS_LIST are managed 
	- just import, give unique name and add
- simplified way test_run for viz is accedded
    - func = locals()["run"] # main func dynamically 	run_test_viz(func)
- viz_d3packhierarchy: 
	- basic working model for viz_d3packhierarchy 
...
-  added new static files dir structure in there
...
June 17, 2016
---------------------------------
- loading an onto via import uri doesnt update the ls command
	- fixed
...
May 27, 2016
---------------------------------
- added import command for local files too [from shell]
	- import web
	- import local
	- import repo
...
- fixed error with display of domain/ranges not in ontospy entities
	- eg with schema.org 'dependencies'  
...
* test fails with http://purl.org/dc/terms/AgentClass
	- fixed
* foaf fails with inspect 
	- fixed
...
April 25, 2016
---------------------------------
* improve inspect for ontologies 
	- testing adding serialization section
	- other improvements
* inspect ==* info
...
- *file* command (contains rename and delete) 
...
* importing via 'download' the ACM fails 
	- but if you pass the URI manually it works <http://www.rkbexplorer.com/ontologies/acm>
...
- refactored messages with help_.. mechanism
...
February 3, 2016
---------------------------------
- added a routine to test different RDF serializations 
	- eg so to address http://pliny.cch.kcl.ac.uk/ontology/base.owl
- show namespaces
...
February 1, 2016
---------------------------------
- tree: no need to show IDs anymore
	- solves also inconsistency with N displayed within ls command
- add find command
	- like ls, but with a pattern
...
October 13, 2015
---------------------------------
- added hidden folder /.index/cache
	- updated code for creation 
- added server app
...
October 11, 2015
---------------------------------
- tested out python webserver 
	- idea: put all exports into folder which is then browsed as a server 
...
- fixed error with ontospy-web circular imports
	- now ontospy-web is only standalone
...
- refactored all the code 
	- cleaned up internal modules loading
...
- @todo maybe serialize and triples should work only from within a currentEntity
	- so we can pass also args...
...
- added more methods on Ontology class i.e. what do we want from it? 
	- get all classes it defines
	- get all properties it defines 


June 12, 2015
---------------------------------
- test connecting to a sparql endpoint
	- how to query an endpoint? are the same sparql functions useful?
...
---------------------------------
- matcher:
	- match not only classes but also properties against each other 
...
---------------------------------
* get label method:
	- get the english one by default!
	- so try to the matching with CIDOC 
...
- removed duplicates in foaf
	- due to entities being defined more than once with different Class predicate (e.g. RDF or OWL)


June 5, 2015
---------------------------------
- added v.2 to github DEV branch
	- from now on use branches to do development! 
	- master is still the most recent active version..
...
- added timing info
- get domain and ranges info for objprops
	- attached also to classes
- added method to run sparql query 
- optimise the allsupers/subs methods -- they really slow down queries!
- added PARENTS/CHILDREN as the immediate relatives 
	- then DESCENDANTS/ANCESTORS which are calculated at run time
	- e.g. from class / -- get parent / --recurse up until top! 
...
---------------------------------
- all supers / all subs
	- still using the SPARQL stuff for now. Easier..
...
May 8, 2015
---------------------------------
- progress with ontospy3
	- can handle multiple ontologies 
		- added test RDF file in folder
...
April 7, 2015
---------------------------------
* Fixed issue with missing templates for webviz
	- added data folder to manifest file
	- changed location of viz file to home folder (cross platform)


March 23, 2015
---------------------------------
* updated the import path and removed the `vocabs` library
	- now the best way to import should be this:
...
- began work on ontospy v.2
	- done ontology loading bit
...
- internalize the ontology, and be able to query a knowledge base (or the same ontology file) using that model 
	- e.g. autocomplete on properties (=dont have to remember all the model details)
	- e.g. introspection kind of stuff
