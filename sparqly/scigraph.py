...
extension of sparqly for scigraph endpoint

python scigraph.py -q "select * where {?z a scigraph:Article}"
...
class ScigraphEndpoint(SparqlEndpoint):
	"""
	...
	def __init__(self, prefixes = {}, verbose=False):
		...
		prefixes = {
			...
			"scig": "http://ns.nature.com/scigraph/",
	...
	def getArticlesForSubject(self, sub_uri):
		...
		results = self.query("""
			SELECT ?article
			WHERE { ?article a scigraph:Article . ?article scigraph:hsaTopic %s}
		""" % subject)
